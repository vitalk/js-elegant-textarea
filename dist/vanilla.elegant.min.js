...
k.innerHTML='&nbsp;'
if(l)m.appendChild(l);m.appendChild(k);return m;};function apply(el,props){for(var prop in props){el.style[camelize(prop)]=props[prop];}};function camelize(s){return s.replace(/-([a-z]|[0-9])/ig,function(s,l){return l.toUpperCase();});};function getHeight(el){return w.getComputedStyle(el).height};function elegant(el,opts){return new Elegant(el,opts)};function Elegant(el,opts){this.opts=opts||{};this.sel=getSelectorEngine(this.opts.sel);if(el){var self=this;if(typeof el==='string'){this.ready(function(){for(var i=0,els=self.sel(el,root),l=els.length;i<l;i++){elegant(els[i],opts)}})}else if(typeof el==='function'){return this.ready(el)}else{this.el=el;if(this.opts.minHeight){this.minHeight=(this.opts.minHeight=='original')?getHeight(el):this.opts.minHeight}else{this.minHeight=0}
...
return ready?fn.call(doc):funcs.push(fn);};return elegant;}());w.Elegant=Elegant;}(this));